### Evol Apps stats analysis for clutch and hatchling level phenotypes ###

# Created by Chris, edited by Charley, 16th May 2024
...
# Scutes
# Scute_total= Scute_C+Scute_L+Scute_R
# 
# data["Scute_total"]=Scute_total
...
# Full starting model
model1 = lmer(data1$SCL_mm ~ data1$Treatment + data1$Split_clutch + model$residuals + 
                data1$Treatment*data1$Split_clutch + data1$Treatment*model$residuals +
...
model2=lmer(data1$SCL_mm ~ data1$Treatment + (1 | factor(data1$Maternal_ID)))
summary(model2)
anova(model2)
...
# SCL vs clutch size
ggplot(data1, aes(x = Split_clutch, y = SCL_mm, color= Treatment)) + 
  geom_point(size=2)  +
  geom_smooth(method = lm)+
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x="Sub-clutch size", y="Hatchling straight carapace length (mm)") +
  theme(text = element_text(size = 15))+
  scale_color_manual(values = c("Deep" = "#0D0887", "Shallow" = "#FA9E3B"))

# SCL vs treatment, by maternal ID
# As maternal ID is included as a random factor in model, box plot should be plot by maternal ID to show differences better
# NB. Only 8 maternal IDs were included in stats, as only those had temp loggers and were kept
# Plot all data:
ggplot(data, aes(x = as.factor(Maternal_ID), y = SCL_mm, fill= Treatment)) + 
  geom_boxplot() +
  labs(x="Maternal ID", y="Hatchling straight carapace length (mm)") +
...
# take residuals of correlation between Temperature and Treatment
model=lm(data1$Full_Incubation_MeanTemp~data1$Treatment)
anova(model)

# Full starting model
model1 = lmer(data1$SCL_mm ~ data1$Treatment + data1$Split_clutch + model$residuals + data1$Weight_g +
                data1$Treatment:data1$Split_clutch + data1$Treatment:model$residuals + data1$Treatment:data1$Weight_g +
                (1| factor(data1$Maternal_ID)))
step(model1)

# Final, stepwise-selected model
model2 = lmer(data1$SCL_mm ~ data1$Treatment + data1$Split_clutch + data1$Weight_g + (1 | factor(data1$Maternal_ID)) + 
                data1$Treatment:data1$Weight_g + 
                data1$Treatment:data1$Split_clutch)
anova(model2)

###### Post hoc ######

# # Visualise fitted lines
# emmip(model2, Treatment ~ Split_clutch, cov.reduce = range)
# emmip(model2, Treatment ~ Weight_g, cov.reduce = range)
# # Summary of contrasts at max and min of range, by treatment
# # Clutch size
# emm <- emmeans(model2, pairwise ~ Treatment * Split_clutch, type = "response", cov.reduce = range)
# pairs(emm, simple = "Treatment")
# # Weight
# emm <- emmeans(model2, pairwise ~ Weight_g * Treatment, type = "response", cov.reduce = range )
# pairs(emm, simple = "Treatment")

### lstrends to estimate and compare average slopes of fitted lines per treatment ###
lstrends(model2, ~ Treatment, var="data1$Weight_g") # Mass
lstrends(model2, ~ Treatment, var="data1$Split_clutch") # Sub-clutch size

### Post hoc: test if slopes are sig different from 0 in each treatment ###
deep <- data1[data1$Treatment == "Deep",]
shallow <- data1[data1$Treatment == "Shallow",]

# Mass 
model_deep = lmer(deep$SCL_mm ~ deep$Weight_g + (1 | factor(deep$Maternal_ID)))
anova(model_deep)
# fixef(model_deep) # Get model slopes

model_shallow = lmer(shallow$SCL_mm ~ shallow$Weight_g + (1 | factor(shallow$Maternal_ID)))
anova(model_shallow)

# Sub-clutch size
model_deep = lmer(deep$SCL_mm ~ deep$Split_clutch + (1 | factor(deep$Maternal_ID)))
anova(model_deep)

model_shallow = lmer(shallow$SCL_mm ~ shallow$Split_clutch + (1 | factor(shallow$Maternal_ID)))
anova(model_shallow)
...
# Interaction between SCL and treatment
ggplot(data, aes(x = Split_clutch, y = SCL_mm, color= Treatment)) + 
  geom_point(size=1)  +
  geom_smooth(method = lm)+
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(x="Sub-clutch size (eggs)", y="Straight carapce length (mm)") +
  theme(text = element_text(size = 15))+
  scale_color_manual(values = c("Deep" = "#0D0887", "Shallow" = "#FA9E3B"))
...
# anova(model2)
# 
# model3=lmer(data1$SCL_mm ~ model2$residuals + model$residuals* data1$Treatment*data1$ Split_clutch+ (1| factor(data1$Maternal_ID)))
# step(model3)
# 
# model4=lmer(data1$SCL_mm ~ model2$residuals + data1$Treatment + data1$Split_clutch + (1 | factor(data1$Maternal_ID)) + data1$Treatment:data1$Split_clutch)
# anova(model4)
# 
# model5=lmer(data1$SCL_mm ~  data1$Treatment*data1$ Split_clutch+ (1| factor(data1$Maternal_ID)))
# anova(model5)
# 
# model6=lmer(data1$SCL_mm ~ model2$residuals + model$residuals+ data1$Treatment+data1$ Split_clutch+ (1| factor(data1$Maternal_ID)))
# anova(model6)
# 
# model7=lmer(data1$SCL_mm ~ model$residuals+ data1$Treatment+data1$ Split_clutch+ (1| factor(data1$Maternal_ID)))
# anova(model7)
# 
# # Interaction between clutch size and temperature
# library("plot3D")
# ## plotted for response to reviewer
# scatter3D(model$residuals, data1$Split_clutch, data1$SCL_mm, clab = c("SCL", "(mm)"), phi=0, bty="b2", xlab= "Mean Temperature (corrected by Treatment)", ylab="Sub-clutch size", zlab="Size in mm")
# scatter3D(model$residuals, data1$Split_clutch, data1$SCL_mm, clab = c("SCL", "(mm)"), phi=0, bty="b2", xlab= "Mean Temperature", ylab="Sub-clutch Size", zlab="SCL (mm)") ## plotted for response to reviewer
...
model1=lmer(log(data2$Run_average+1) ~ data2$Treatment + data2$Split_clutch + model$residuals + data2$SCL_mm + 
              data2$Treatment:data2$Split_clutch + data2$Treatment:model$residuals + data2$Treatment:data2$SCL_mm +
              (1| factor(data2$Maternal_ID)))

step(model1)

# Final, stepwise-selected model
model2=lmer(log(data2$Run_average + 1) ~ data2$Treatment + data2$Split_clutch + model$residuals + (1 | factor(data2$Maternal_ID)) + 
              data2$Treatment:data2$Split_clutch + data2$Treatment:model$residuals)
...
### lstrends to estimate and compare average slopes of fitted lines per treatment ###
lstrends(model2, ~ Treatment, var="model$residuals") # Temp residuals
lstrends(model2, ~ Treatment, var="data2$Split_clutch") # Sub-clutch size

### Post hoc: test if slopes are sig different from 0 in each treatment ###
deep <- data2[data2$Treatment == "Deep",]
shallow <- data2[data2$Treatment == "Shallow",]

# Temp
model_deep = lmer(log(deep$Run_average + 1) ~ deep$Full_Incubation_MeanTemp + (1 | factor(deep$Maternal_ID)))
anova(model_deep)

model_shallow = lmer(log(shallow$Run_average + 1) ~ shallow$Full_Incubation_MeanTemp + (1 | factor(shallow$Maternal_ID)))
anova(model_shallow)

# Sub-clutch size
model_deep = lmer(log(deep$Run_average + 1) ~ deep$Split_clutch + (1 | factor(deep$Maternal_ID)))
anova(model_deep)

model_shallow = lmer(log(shallow$Run_average + 1) ~ shallow$Split_clutch + (1 | factor(shallow$Maternal_ID)))
anova(model_shallow)
...
#plotting run time by clutch size split for treatment 
ggplot(data2, aes(x = Split_clutch, y = log(Run_average+1), color = Treatment)) + 
  geom_point(size=2)  +
  geom_smooth(method = lm)+
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
# ### Response to reviewer ###
# # exclude temperature
# model1=lmer(log(data2$Run_average+1) ~ data2$SCL_mm+ data2$Split_clutch * data2$Treatment+ (1| factor(data2$Maternal_ID))) #### note here SCL is simply added as a covariate
# step(model1)
# 
# model2=lmer(log(data2$Run_average + 1) ~ data2$Split_clutch + data2$Treatment + (1 | factor(data2$Maternal_ID)) + data2$Split_clutch:data2$Treatment)
# anova(model2)
# 
...
# Full, starting model
# SCL used as a covariable
model1=lmer(log(data3$Flip_average+1) ~ data3$Treatment + model$residuals + data3$Split_clutch + data3$SCL_mm +
              data3$Treatment:model$residuals + data3$Treatment:data3$Split_clutch + data3$Treatment:data3$SCL_mm +
...
### Plots ###
ggplot(data, aes(x = Treatment, y = log(Flip_average+1), fill = Treatment)) + 
...
# Split by maternal ID
ggplot(data, aes(x = as.factor(Maternal_ID), y = log(Flip_average+1), fill = Treatment)) + 
  geom_boxplot(alpha=0.85)  +
  labs(x="Maternal ID", y="Time to self-righting (log+1) in seconds") +
  scale_fill_manual(values=c("#FA9E3B","#0D0887")) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme_classic() +
  theme(axis.title = element_text(size = 16), axis.text = element_text(size = 14), 
